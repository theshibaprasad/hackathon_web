...
async function addSampleData() {
  try {
    ...
    const problemStatementsToCreate = sampleProblemStatements.map(ps => ({
      title: ps.title,
      description: ps.description,
      themeId: createdThemes[ps.themeIndex]._id
    }));

    const createdProblemStatements = await ProblemStatement.insertMany(problemStatementsToCreate);
    console.log(`✅ Created ${createdProblemStatements.length} problem statements`);

    console.log('\n🎉 Sample data added successfully!');
    console.log('\nYou can now:');
    console.log('1. Visit /problem-statements to see the public page');
    console.log('2. Visit /admin/dashboard to manage themes and problem statements');
